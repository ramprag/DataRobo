...
class PrivacyMasker:
    ...
    def apply_privacy_masks(self, df: pd.DataFrame, privacy_config) -> pd.DataFrame:
        ...
        try:
            logger.info("Starting privacy masking process")
            masked_df = df.copy()
            ...
            if hasattr(privacy_config, 'custom_fields') and privacy_config.custom_fields:
            ...
            logger.info("Privacy masking completed successfully")
            return masked_df

        except Exception as e:
            logger.error(f"Error in privacy masking: {e}")
            raise
    ...
    def _mask_emails(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            for column in email_columns:
                logger.info(f"Masking email column: {column}")
            ...
            logger.error(f"Error masking emails: {e}")
            return df
    ...
    def _mask_names(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            for column in name_columns:
                logger.info(f"Masking name column: {column}")
            ...
            logger.error(f"Error masking names: {e}")
            return df
    ...
    def _mask_phone_numbers(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            for column in phone_columns:
                logger.info(f"Masking phone column: {column}")
            ...
            logger.error(f"Error masking phone numbers: {e}")
            return df
    ...
    def _mask_addresses(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            for column in address_columns:
                logger.info(f"Masking address column: {column}")
            ...
            logger.error(f"Error masking addresses: {e}")
            return df
    ...
    def _mask_ssn(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            for column in ssn_columns:
                logger.info(f"Masking SSN column: {column}")
            ...
            logger.error(f"Error masking SSN: {e}")
            return df
            ...
            for column in df.columns:
                if column not in matching_columns:
                    try:
                        ...
                        if sample_values:
                            match_count = sum(1 for val in sample_values if re.search(pattern, str(val)))
                            # If more than 30% of sample matches, consider it a match
                            if match_count / len(sample_values) > 0.3:
                                matching_columns.append(column)
                    except Exception as e:
                        logger.warning(f"Error checking pattern for column {column}: {e}")
    ...
    def _mask_column(self, df: pd.DataFrame, column: str, method: str = 'faker') -> pd.DataFrame:
        ...
        try:
            ...
            logger.info(f"Masking custom column: {column} using method: {method}")
            ...
            logger.error(f"Error masking column {column}: {e}")
            return df
    ...
    def _mask_email_value(self, value):
        ...
        try:
            ...
            value_str = str(value)
            if re.search(self.pii_patterns['email'], value_str):
    ...
    def _mask_ssn_value(self, value):
        ...
        try:
            ...
            value_str = str(value)
            if re.search(self.pii_patterns['ssn'], value_str):
